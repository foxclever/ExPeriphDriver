...
/*计算AD通道数据输出建立时间*/
static uint32_t CalcDataOutputTime(Ad7192ObjectType *adObj);

/*连续转换数据获取,dataCodes为8个元素的数组对应8个通道*/
void GettContinuousConvertionValue(Ad7192ObjectType *adObj,uint32_t Channels)
{
    uint32_t dataCode=0;
    uint16_t number=0;
    uint8_t status=0xFF;
    uint32_t ch[]={CHAIN1AIN2,CHAIN3AIN4,CHTEMP,CHAIN2AIN2,
    CHAIN1COM,CHAIN2COM,CHAIN3COM,CHAIN4COM};
    
    AD7192StartContinuousConvertion(adObj,Channels&0x00FF00);
    
    for(int i=0;i<8;i++)
    {
        if((Channels&ch[i])==ch[i])
        {
            number++;
        }
    }
    ...
    for(int i=0;i<number;i++)
    {
        ...
        adObj->dataCodes[status]=dataCode;
    }
}
...
/*AD7192软件复位，连续发送40个1就会复位*/
void AD7192SoftwareReset(Ad7192ObjectType *adObj)
{
    uint8_t WriteBuf[1];
    uint8_t ReadBuf[1];
    uint8_t i;
...
static uint32_t AD7192ReadConvertingData(Ad7192ObjectType *adObj)
{
    ...
    adObj->Delay(CalcDataOutputTime(adObj));
//    while(adObj->GetReadyInput()== 0){;}			
//    while(adObj->GetReadyInput()== 1){;}			//等待第一个RDY下降沿
...
static uint32_t CalcDataOutputTime(Ad7192ObjectType *adObj)
{
    ...
    if((adObj->Registers[AD7192_REG_MODE]&SINC_3)==SINC_3)
    {
        calcTime=3072.0/4920.0*adObj->fs;
    }
    else
    {
        calcTime=4096.0/4920.0*adObj->fs;
    }
    
    if((adObj->Registers[AD7192_REG_CONF]&CHOP_EN)==CHOP_EN)
    {
        calcTime=calcTime*2;
    }
    
    outTime=(uint32_t)calcTime+1;
...
static void AD7192StartSingleConvertion(Ad7192ObjectType *adObj,uint32_t Channels)
{
    ...
    adObj->Registers[AD7192_REG_MODE] = 0;
    adObj->Registers[AD7192_REG_MODE] = MODE_SING|DAT_STA_EN|INCLK_MCLK2EN|SINC_4|ENPAR_EN|CLK_DIV_DIS|SINGLECYCLE_DIS|REJ60_DIS|adObj->fs;
    WriteAD7192Register(adObj,AD7192_REG_MODE, 1);
}
