...
#define DPI_RH8_T13     0x01    //湿度8位温度13位精度
#define DPI_RH10_T12    0x80    //湿度10位温度12位精度
...
const uint16_t SHT2xPOLYNOMIAL = 0x31;      ////定义校验多项式P(x) = x^8 + x^5 + x^4 + 1 = 100110001
...
static float CalcSHT2xTemperature(uint16_t sTemp)
{
  ...
  temperatureC = -46.85 + (float)sTemp*(175.72 / 65536);
...
static float CalcSHT2xHumidity(uint16_t sHumi)
{
  ...
  sHumi &= 0xFFFC;
  /* RH= -6 + 125 * SRH/2^16*/
  humidityRH = -6.0 + (float)sHumi*(125.0/65536); 
